...
hostname = CONSTANTS["postgres_host"]
...
def desp_preprocessing(text):
    ...
    if language_full_form.lower() in stemmed_languages:
        ...
        pattern = (
            r"\b(?:" + "|".join(stopwords_by_language) + r")\b"
    ...
    for position, word in enumerate(words):
        if word not in positional_index:
            positional_index[word] = {
                "df": 0,
                "posting_list": {},
            }  # add the entry for the word if word is not contained
        if (
            id not in positional_index[word]["posting_list"]
        ):  # if it is the first appearence from the  word in the doc
            positional_index[word]["df"] += 1  # the df is incremented
            positional_index[word]["posting_list"][
                id
            ] = []  # a list is created to store the positions
...
def create_index():
    ...
    uri = (
        ...
        + ":"
        + PASSWORD
        + "@ttds-cluster.vubotvd.mongodb.net/?retryWrites=true&w=majority"
    )
